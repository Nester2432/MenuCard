...
>(({ className, variant = "default", ...props }, ref) => (
  <ToastPrimitive.Root
    ref={ref}
    className={cn(
      "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-2xl border p-4 pr-8 shadow-elevated transition-all",
      variant === "destructive" ? "border-red-200 bg-red-50 text-red-800" : "border-neutral-100 bg-white text-neutral-900",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitive.Close
    ref={ref}
    className={cn("absolute right-2 top-2 rounded-lg p-1 text-neutral-400 hover:text-neutral-600 transition-colors", className)}
    {...props}
  >
    <X className="h-4 w-4" />
  </ToastPrimitive.Close>
